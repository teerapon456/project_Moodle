...
try {
    ...
    $sql = "
        SELECT
            ...
            CASE 
                WHEN trqs.is_random_mode = 1 THEN 
                    -- For random mode, calculate the max score based on the actual questions shown to the user
                    (SELECT COALESCE(SUM(q.score), 0) 
                     FROM iga_user_answers ua 
                     JOIN iga_questions q ON ua.question_id = q.question_id 
                     WHERE ua.attempt_id = uta.attempt_id)
                ELSE 
                    -- For regular mode, use the total score of all questions
                    COALESCE(test_max_scores.max_test_score, 0) 
            END AS max_test_score
        FROM iga_user_test_attempts uta
        JOIN users u ON uta.user_id = u.user_id
        JOIN iga_tests t ON uta.test_id = t.test_id
        JOIN roles r ON u.role_id = r.role_id
        LEFT JOIN iga_test_random_question_settings trqs ON t.test_id = trqs.test_id
        LEFT JOIN ( -- Subquery เพื่อรวมคะแนนสูงสุดที่เป็นไปได้ของแต่ละแบบทดสอบ (สำหรับแบบปกติ)
            SELECT
                s.test_id,
    ...
    if ($pass_fail_filter !== '-1') {
        ...
        $where_clauses[] = "(CASE 
            WHEN trqs.is_random_mode = 1 AND uta.is_completed = 1 THEN 
                (SELECT CASE 
                    WHEN COALESCE(SUM(q.score), 0) > 0 
                    THEN (uta.total_score / SUM(q.score) * 100) >= t.min_passing_score 
                    ELSE 0 
                 END 
                 FROM iga_user_answers ua 
                 JOIN iga_questions q ON ua.question_id = q.question_id 
                 WHERE ua.attempt_id = uta.attempt_id)
            WHEN COALESCE(test_max_scores.max_test_score, 0) > 0 AND uta.is_completed = 1 
                THEN (uta.total_score / test_max_scores.max_test_score * 100) >= t.min_passing_score 
            ELSE 0 
        END) = ?";

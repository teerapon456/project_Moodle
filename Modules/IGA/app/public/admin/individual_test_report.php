...
// Legend Histogram แยก 3 รายการ: Frequency (แท่ง) / Normal curve (เส้นระฆัง) / You are here (จุด)
...
try {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // auto-graded ต่อ section (เฉพาะ MC/TF)
    ...
    plugins: {
      ...
      tooltip: { callbacks: {
        label: (ctx) => {
          ...
          return label + new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(v) + ' <?php echo get_text('score_label_suffix'); ?>';
        }
      }},
      datalabels: {
...
function renderHistogramWithNormal(allScores, curScore, fullInt){
  ...
  // ---- เส้นระฆัง "ทฤษฎี" ให้อยู่กึ่งกลางคะแนนเต็มเสมอ ----
  const n = (allScores || []).length;
  const meanTheo = N / 2;
  const sdTheo   = Math.max(N / 6, 1e-6); // ±3σ ≈ 0..N
  ...
  if (n > 0) {
    for (let k=0; k<=N; k++){
      const xMid = (k + 0.5) * binWidth;        // กึ่งกลาง bin
      curveAtBins[k] = pdfNorm(xMid, meanTheo, sdTheo) * n * binWidth; // สเกลเป็น "คนต่อ bin"
    }
  }

  // y-axis ให้ครอบทั้งแท่งและเส้น
...
function rerenderCurrentChart(){
  ...
  if (type === 'normal') { setLegendVisible(false); renderNormalCurve(allAttemptScores, currentAttemptScore, FULL_INT); return; }

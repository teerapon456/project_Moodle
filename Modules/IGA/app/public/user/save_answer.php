...
try {
    ...
    $stmt = $conn->prepare("
        ...
        JOIN iga_questions q ON q.question_id = ?
    ...
    $stmt->bind_param("iis", $question_id, $attempt_id, $user_id);
    $stmt->execute();
    ...
    if ($question_type === 'multiple_choice' || $question_type === 'true_false') {
        ...
        $correct_option_stmt = $conn->prepare("SELECT option_id FROM iga_question_options WHERE question_id = ? AND is_correct = 1");
    ...
    //    ต้องแน่ใจว่าตาราง iga_user_answers มี UNIQUE KEY (attempt_id, question_id)
    $stmt = $conn->prepare("
        INSERT INTO iga_user_answers (attempt_id, question_id, user_answer_text, is_correct, score_earned)
        VALUES (?, ?, ?, ?, ?)
    ...
    if (!$stmt) {
        ...
        throw new Exception("Database prepare error for iga_user_answers. Please check server logs.");
    }
    ...
    if ($stmt->affected_rows > 0 || $stmt->insert_id > 0) {
        // 4. อัพเดตคะแนนรวมใน iga_user_test_attempts ทันที
        ...
        $total_score_update_stmt = $conn->prepare("
            ...
            SET total_score = (SELECT COALESCE(SUM(score_earned), 0) FROM iga_user_answers WHERE attempt_id = ?)
            WHERE attempt_id = ?
        ");

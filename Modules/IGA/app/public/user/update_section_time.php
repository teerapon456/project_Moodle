...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt_check = $conn->prepare("SELECT COUNT(*) FROM iga_user_section_times WHERE attempt_id = ? AND section_id = ?");
        ...
        if ($row_count > 0) {
            // อัพเดทเวลาที่ใช้ไป
            $stmt_update = $conn->prepare("UPDATE iga_user_section_times SET time_spent_seconds = ? WHERE attempt_id = ? AND section_id = ?");
            ...
            $stmt_insert = $conn->prepare("INSERT INTO iga_user_section_times (attempt_id, section_id, time_spent_seconds) VALUES (?, ?, ?)");

...
if (!has_role('admin') && !has_role('super_user') && !has_role('editor')) {
    ...
    header("Location: /INTEQC_GLOBAL_ASSESMENT/login"); // หรือ ../views/user/dashboard.php
    exit();
}
...
try {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) AS published_tests FROM tests WHERE is_published = 1");
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) AS total_associates FROM users WHERE role_id = (SELECT role_id FROM roles WHERE role_name = 'associate')");
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    // จำนวนผู้ใช้งานทั้งหมด (ไม่รวม admin)
    $stmt = $conn->prepare("SELECT COUNT(*) AS total_applicants FROM users WHERE role_id = (SELECT role_id FROM roles WHERE role_name = 'applicant')");
    $stmt->execute();
    $result = $stmt->get_result();
    $total_applicants = $result->fetch_assoc()['total_applicants'];
    $stmt->close();

    // จำนวนผู้ใช้งานที่ทำแบบทดสอบแล้ว (นับจาก user_test_attempts ที่ is_completed = 1)
    $stmt = $conn->prepare("SELECT COUNT(DISTINCT user_id) AS users_completed_test FROM user_test_attempts WHERE is_completed = 1");
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    // จำนวนผู้ใช้งานที่ทำแบบทดสอบแล้ว (นับจาก user_test_attempts ที่ is_completed = 1)
    $stmt = $conn->prepare("SELECT COUNT(DISTINCT test_id) AS tests_completed FROM user_test_attempts WHERE is_completed = 1");
    $stmt->execute();
    $result = $stmt->get_result();
...
<div class="row equal-height-row">
    <div class="col-md-6 col-lg-3 mb-4">
        <div class="card text-white" style="background: #4ECDC4 !important; border: none;"  shadow <?php echo $is_super_user ? 'disabled-card' : ''; ?>">
            <div class="card-body">
                <div class="card-icon">
            ...
            <div class="card-footer" style="background: rgba(0, 0, 0, 0.1) !important; border-top: 1px solid rgba(255, 255, 255, 0.2);">
                <a href="/INTEQC_GLOBAL_ASSESMENT/admin/tests" class="text-white text-decoration-none <?php echo $is_super_user ? 'disabled-link' : ''; ?>">
    ...
    <div class="col-md-6 col-lg-3 mb-4">
        <div class="card text-white" style="background: #FF9F1C !important; border: none;"  shadow <?php echo $is_super_user ? 'disabled-card' : ''; ?>">
            <div class="card-body">
                <div class="card-icon">
            ...
            <div class="card-footer" style="background: rgba(0, 0, 0, 0.1) !important; border-top: 1px solid rgba(255, 255, 255, 0.2);">
                <form action="/INTEQC_GLOBAL_ASSESMENT/admin/tests" method="POST" style="display: inline;">
                    <input type="hidden" name="published" value="1">
                    <?php echo generate_csrf_token(); ?>
                    <button type="submit" class="btn btn-link text-white text-decoration-none p-0 <?php echo $is_super_user ? 'disabled-link' : ''; ?>" <?php echo $is_super_user ? 'disabled' : ''; ?>>
                        <?php echo get_text('view_details_link', []); ?> <i class="fas fa-arrow-circle-right"></i>
                    </button>
                </form>
            </div>
    ...
    <div class="col-md-6 col-lg-3 mb-4">
        <div class="card text-white" style="background: #7AC74F !important; border: none;"  shadow <?php echo $is_super_user || $is_editor ? 'disabled-card' : ''; ?>">
            ...
            <div class="card-footer" style="background: rgba(0, 0, 0, 0.1) !important; border-top: 1px solid rgba(255, 255, 255, 0.2);">
                <a href="/INTEQC_GLOBAL_ASSESMENT/admin/reports" class="text-white text-decoration-none <?php echo $is_super_user || $is_editor ? 'disabled-link' : ''; ?>">
                    <?php echo get_text('view_details_link', []); ?> <i class="fas fa-arrow-circle-right"></i>
                </a>
            </div>
        </div>
    ...
    <div class="col-md-6 col-lg-3 mb-4">
        <div class="card text-white" style="background: #FF4D6D !important; border: none;"  shadow <?php echo $is_super_user ? 'disabled-card' : ''; ?>">
            <div class="card-body">
                <div class="card-icon">
            ...
            <div class="card-footer" style="background: rgba(0, 0, 0, 0.1) !important; border-top: 1px solid rgba(255, 255, 255, 0.2);">
                <a href="/INTEQC_GLOBAL_ASSESMENT/admin/review-answers" class="text-white text-decoration-none <?php echo $is_super_user ? 'disabled-link' : ''; ?>">
    ...
    <div class="col-md-6 col-lg-3 mb-4">
        <div class="card text-white" style="background: #B388FF !important; border: none;"  shadow <?php echo $is_super_user || $is_editor ? 'disabled-card' : ''; ?>">
            ...
            <div class="card-footer" style="background: rgba(0, 0, 0, 0.1) !important; border-top: 1px solid rgba(255, 255, 255, 0.2);">
                <a href="/INTEQC_GLOBAL_ASSESMENT/admin/users" class="text-white text-decoration-none <?php echo $is_super_user || $is_editor ? 'disabled-link' : ''; ?>">
                    <?php echo get_text('view_details_link', []); ?> <i class="fas fa-arrow-circle-right"></i>
                </a>
            </div>
        </div>

...
if (isset($_SESSION['user_id']) && $conn) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['excel_file'])) {
        ...
        try {
            ...
            if (isset($testData[1])) {
                ...
                $stmt_test = $conn->prepare("INSERT INTO tests (test_name, description, is_published, created_by_user_id, duration_minutes, show_result_immediately, min_passing_score, creation_year, test_no, language, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
                $stmt_test->bind_param(
                    "ssiisiiiss", // s: test_name, s: description, i: is_published, i: created_by_user_id, i: duration_minutes, i: show_result_immediately, i: min_passing_score, i: creation_year, s: test_no, s: language
                    $test_name,
                    $description,
                    $is_published,
                    $created_by_user_id,
            ...
            for ($row_idx = 2; $row_idx <= $maxRowIndex; $row_idx++) { // Use <= $maxRowIndex
                ...
                if (!empty($section_name_from_excel) && ($section_name_from_excel !== $current_section_name || ($section_order_from_excel > 0 && $section_order_from_excel !== $current_section_order) || $current_section_id === null)) {
                    ...
                    $stmt_section = $conn->prepare("INSERT INTO sections (test_id, section_name, description, duration_minutes, section_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())");
                ...
                // --- Question Insertion ---
                $stmt_question = $conn->prepare("INSERT INTO questions (section_id, question_text, question_type, score, question_order, is_critical, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())");
                ...
                if ($question_type === 'multiple_choice') {
                    ...
                    foreach ($options_to_insert as $option_data) {
                        $stmt_option = $conn->prepare("INSERT INTO question_options (question_id, option_text, is_correct, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())");
                    ...
                    foreach ($tf_options as $option_key => $option_text) {
                        ...
                        $stmt_tf_option = $conn->prepare("INSERT INTO question_options (question_id, option_text, is_correct, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())");
            ...
            header("Location: /INTEQC_GLOBAL_ASSESMENT/admin/tests"); // Redirect to a success page
            exit();
        } catch (Exception $e) {
            ...
            header("Location: /INTEQC_GLOBAL_ASSESMENT/admin/import_test_form.php");
            exit();
        }
    } else {
    ...
    set_alert(get_text('error_user_not_logged_in_for_import'), "danger");
    header("Location: /INTEQC_GLOBAL_ASSESMENT/admin/login.php");
    exit();
}

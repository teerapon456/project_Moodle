...
if (!is_numeric($attempt_id) || $attempt_id <= 0) {
    ...
    header("Location: /INTEQC_GLOBAL_ASSESMENT/admin/review-answers/");
    exit();
}
...
try {
    ...
    $stmt_test = $conn->prepare("SELECT test_name FROM tests WHERE test_id = ?");
    ...
    $stmt_qa = $conn->prepare("
        ...
        WHERE
            ua.attempt_id = ? AND q.question_type = 'short_answer'
        ORDER BY
            q.question_order ASC
    ");
    $stmt_qa->bind_param("i", $attempt_id);
    $stmt_qa->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['question_scores'])) {
    ...
    try {
        ...
        foreach ($_POST['question_scores'] as $user_answer_id => $score) {
            ...
            if ($score === '' || !is_numeric($score)) {
                ...
                if ($score_to_save > $max_score) {
                    ...
                    header("Location: /INTEQC_GLOBAL_ASSESMENT/admin/review-answer/?attempt_id=" . $attempt_id);
                    exit();
                }
            ...
            $update_stmt = $conn->prepare("UPDATE user_answers SET score_earned = ?, is_correct = ? WHERE user_answer_id = ? AND attempt_id = ?");
        ...
        $update_attempt_score_stmt = $conn->prepare("UPDATE user_test_attempts SET total_score = ? WHERE attempt_id = ?");
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <a href="/INTEQC_GLOBAL_ASSESMENT/admin/review-answers/" class="btn btn-outline-secondary">
                    ...
                    <div class="mb-4 p-3 border rounded">
                        ...
                        <?php if ($qa['score_earned'] !== NULL): ?>
                            <p class="text-muted mt-2">
                                <i class="fas fa-history me-1"></i> <?php echo get_text('reviewed_status_label'); ?> <?php echo get_text('scored_points_label', number_format($qa['score_earned'], 2), number_format($qa['question_max_score'], 2)); ?>
                                <?php
                                if ($qa['is_correct'] === 1) {
                                    echo '<span class="badge bg-success ms-2">' . get_text('correct_status') . '</span>';
                                } elseif ($qa['score_earned'] > 0 && $qa['score_earned'] < $qa['question_max_score']) {
                                    echo '<span class="badge bg-info ms-2">' . get_text('partially_correct_status') . '</span>';
                                } else {
                                    echo '<span class="badge bg-danger ms-2">' . get_text('incorrect_status') . '</span>';
                                }
                                ?>
                            </p>
                        <?php endif; ?>
        ...
        scoreInputs.forEach(input => {
            input.addEventListener('input', function() {
                ...
                if (currentScore > maxScore) {
                    ...
                    feedbackElement.textContent = <?php echo get_text('score_exceeds_max'); ?>;

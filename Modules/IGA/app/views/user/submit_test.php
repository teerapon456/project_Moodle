...
if (!has_role('associate') && !has_role('applicant')) {
    ...
    redirect_to('/INTEQC_GLOBAL_ASSESMENT/user/');
    exit();
}
...
try {
    ...
    ");
    $stmt->bind_param("sdiis", $end_time, $final_score, $time_spent_actual, $attempt_id, $user_id);
    $stmt->execute();
    ...
    if ($stmt->affected_rows > 0) {
        ...
        if ($show_result_immediately) {
            ...
            echo '<form id="redirect_form" action="/INTEQC_GLOBAL_ASSESMENT/user/results" method="post">';

...
try {
    ...
    $stmt = $conn->prepare("SELECT attempt_id FROM user_test_attempts WHERE attempt_id = ? AND user_id = ? AND is_completed = 0");
    if (!$stmt) {
    ...
    $stmt->bind_param("iii", $current_question_index, $time_spent_seconds_overall, $attempt_id);
    $stmt->execute();
    ...
    if (!is_null($section_id) && !is_null($time_spent_in_section)) {
        ...
        $stmt_check_section_time = $conn->prepare("SELECT section_time_id FROM user_section_times WHERE attempt_id = ? AND section_id = ?");
        ...
        if ($result_check->num_rows > 0) {
            ...
            $stmt_update_section_time = $conn->prepare("UPDATE user_section_times SET time_spent_seconds = ? WHERE attempt_id = ? AND section_id = ?");
            ...
            $stmt_insert_section_time = $conn->prepare("INSERT INTO user_section_times (attempt_id, section_id, time_spent_seconds) VALUES (?, ?, ?)");
